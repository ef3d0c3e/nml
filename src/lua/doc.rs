...
pub static LUA_DOCS: LazyLock<Mutex<HashMap<&'static str, UserDataDoc>>> =
...
macro_rules! add_lua_docs {
	($t:ty) => {{
		...
		lua.register_userdata_type::<$t>(|reg| <$t as mlua::UserData>::add_methods(reg))
			.unwrap();
		0
	}};
}
...
pub fn get_lua_docs() {
	...
	fn document_function(name: &'static str, fun: &LuaFunc) -> String {
		...
		buf += &format!("function nml.{}({args}) end\n\n", name);
		buf
	}
